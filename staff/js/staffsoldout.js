class StaffSoldout {
  ...
  initialize() {
    const soldOutBtn = document.getElementById("viewSoldOutBtn");
    if (soldOutBtn) {
      soldOutBtn.addEventListener("click", () => this.showSoldOutModal());
    } else {
      console.error("Sold out button not found");
    }
  }
  ...
  async showSoldOutModal() {
    try {
      ...
      this.modal.className = "staffsoldout-modal";
      this.modal.innerHTML = `
        <div class="staffsoldout-content">
          <div class="staffsoldout-header">
            <h2>Sold Out Items</h2>
            <div class="staffsoldout-actions">
              <button class="staffsoldout-export">Export PDF</button>
              <button class="staffsoldout-close">&times;</button>
            </div>
          </div>
          <div class="staffsoldout-filters">
            <select class="staffsoldout-week-filter">
              ${weeks
                .map(
                  (week) => `
          ...
          <div class="staffsoldout-table-container">
            <table class="staffsoldout-table">
              <thead>
                <tr>
                  <th>Code</th>
                  <th>Name</th>
  ...
  setupEventListeners(items) {
    const closeBtn = this.modal.querySelector(".staffsoldout-close");
    closeBtn.onclick = () => this.modal.remove();

    const weekSelect = this.modal.querySelector(".staffsoldout-week-filter");
    weekSelect.addEventListener("change", (e) => {
      const weekStart = new Date(e.target.value);
      const tbody = this.modal.querySelector(".staffsoldout-table tbody");
    ...
    const exportBtn = this.modal.querySelector(".staffsoldout-export");
